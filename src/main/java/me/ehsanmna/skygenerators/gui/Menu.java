...
public class Menu implements Cloneable {
    ...
    private ItemStack processItemStack(ItemStack item, PlayerGenerator generator) {
        ...
        if (meta.getPersistentDataContainer().has(new NamespacedKey(SkyGenerators.getInstance(), "upgrade-slot"))) return processUpgrade(item, generator);
    ...
    private void fillUpgradesItem(Inventory gui, PlayerGenerator generator) {
        for (GeneratorUpgrade generatorUpgrade : generator.getUpgrades()){
            ...
            int generated = generatorUpgrade.getGeneratedAmount();
            int emptySlots = InventoryUtils.getEmptySlotsCount(gui);
            int maxStackable = emptySlots * 64;
            ItemStack blockItem = new ItemStack(generatorUpgrade.getBaseGeneratorUpgrade().getUpgradeBuild().getOutputStack(generatorUpgrade));
